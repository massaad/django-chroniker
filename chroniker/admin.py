...
from chroniker.models import Job, JobDependency, Log, Monitor
...
class JobAdmin(admin.ModelAdmin):
    ...
    def get_timeuntil(self, obj=None):
        ...
        return format_html(
            "%s<br /><span class='mini'>(%s)</span>" % (value, obj.get_timeuntil())
        )
    ...
    def view_logs_button(self, obj=None):
        ...
        return format_html(
            '<a href="{url}?job__id__exact={id}"'
            ' target="_blank" class="button">View&nbsp;{count}</a>'.format(**kwargs)
        )
    ...
    def view_duration_graph(self, request, object_id):
        ...
        if obj:
            ...
            max_duration = q.aggregate(models.Max("duration_seconds"))[
...
class MonitorAdmin(admin.ModelAdmin):
    ...
    def action_buttons(self, obj):
        buttons = []
        buttons.append(
            '<a href="%s" class="button">Check now</a>' % "%d/run/?inline=1" % obj.id
        )
        buttons.append(
            '<a href="/chroniker/job/%i/"'
    ...
    def run_job_view(self, request, pk):
        ...
        job.save()
        self.message_user(
            request, _('The monitor "%(job)s" will be checked.') % {"job": job}
        )
        if "inline" in request.GET:
            redirect = request.path + "../../"
        else:
