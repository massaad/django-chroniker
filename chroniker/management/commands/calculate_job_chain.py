...
class Command(BaseCommand):
    ...
    def handle(self, root_job_id, **options):
        ...
        system.add(
            Node(
                root_job.id, duration=root_job.get_run_length_estimate(samples=samples)
            )
        )
        print(
            "%s takes about %s seconds"
        ...
        for job in chain:
            if not job.enabled:
                continue
            dependees = JobDependency.objects.filter(
                dependent=job, dependee__enabled=True
            )
        ...
        print("min hours:", system.duration * (1 / 60.0) * (1 / 60.0))
